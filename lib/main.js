...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (isMonorepo) {
            if (npmClient !== 'yarn') {
                yield exec_1.exec('./node_modules/.bin/lerna bootstrap');
            }
            yield exec_1.exec('./node_modules/.bin/lerna exec -- eval "[ -f release.config.js ] && ../../node_modules/.bin/semantic-release"');
        }
        else {
            yield exec_1.exec('eval "[ -f release.config.js ] && ./node_modules/.bin/semantic-release"');
        }
